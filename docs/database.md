...
erDiagram
    ...
    SURVEY_BATCHES {
        ...
        string s3_key "保存URI (s3://... or local://...)"
    }

    SURVEY_RESPONSES {
        bigint id PK
        bigint survey_batch_id FK
        int row_index "CSV行番号"
    ...
    COMMENT_SUMMARIES {
        ...
        bigint survey_batch_id FK
        string student_attribute "受講生属性(ALL含む)"
        string analysis_type "sentiment/category/importance/fix_difficulty"
...
1回のExcelアップロードに対応する履歴テーブル。「速報版」と「確定版」は別レコードとして保存される。`(lecture_id, batch_type)` にユニーク制約（`uq_survey_batch_lecture_type`）を持ち、同一講義に対して同じタイプのバッチは1つのみ許容される。
...
| `s3_key` | VARCHAR(1024) | Nullable | アップロードファイルの保存URI（`s3://...` または `local://...`） |
...
| `row_index` | INT | Nullable | CSV元データの行番号 |
| `account_id` | VARCHAR(255) | Not Null | アカウントID |
| `student_attribute`  | VARCHAR(50) | Not Null | 受講生の属性キー（`student`, `corporate`, `invited`, `faculty`, `other`）。未設定時は `ALL` を補完 |
...
| `is_analyzed` | BOOLEAN | Default FALSE | LLM分析済みかどうかのフラグ（現行実装では保存時に `TRUE` を設定） |
...
- 非同期ワーカーで `ResponseComment` を作成する時点で、`llm_...` カラムを同時に保存する（`question_type='learned'` などの非対象設問はフォールバック値で補完）。
- 現行フローでは `is_analyzed` は保存時に `TRUE` となり、`WHERE is_analyzed = FALSE` を後段で再解析する運用は行っていない。
...
| `student_attribute` | VARCHAR(50) | Not Null | 属性（`ALL` または `student` / `corporate` / `invited` / `faculty` / `other`） |
...
選択した講義回の全質問項目の分布を表示するために、設問ごとの点数分布を縦持ちで保存する（5段階評価は `1〜5`、NPSは `0〜10`）。
...
| `score_value` | INT | Not Null | 点数（設問に応じて `1〜5` または `0〜10`） |
...
| `survey_batch_id` | BIGINT | FK | `survey_batches.id` への外部キー |
| `student_attribute` | VARCHAR(50) | Not Null | 属性（`ALL` または `student` / `corporate` / `invited` / `faculty` / `other`） |
| `analysis_type` | VARCHAR(20) | Not Null | 集計タイプ (`sentiment`, `category`, `importance`, `fix_difficulty`) |
