...
- **ベースURL**: `/api/v1`
- **プロトコル**: HTTPS
- **レスポンス形式**: 主に JSON（`/login`, `/logout` はリダイレクトレスポンス）
- **文字コード**: UTF-8
...
| **日時** | ISO 8601 (`YYYY-MM-DDTHH:mm:ss[.fraction][Z]`) | `2024-10-07T09:00:00Z` |

> 補足（現行実装）: DB由来の日時はタイムゾーン表記なし（例: `2024-10-07T09:00:00`）で返る場合がある。
...
本番運用（ALB経由）では、バックエンドに到達する時点で認証済みであることを前提とする。
一方、FastAPIアプリ単体には 401/403 を返す認証ガードは実装していないため、ALBを経由しない直接アクセスでは未認証でもエンドポイントが応答する。
...
開発時は、バックエンドのミドルウェアで上記のヘッダー相当をモックして開発を行う。
`API_DEBUG=true` の場合はモックユーザーが自動注入され、`API_DEBUG=false` の場合は `request.state.user` が空のまま処理が継続される。
...
> 注意（現行実装）: `POST /surveys/upload` のファイルヘッダー検証で失敗した場合、`CsvValidationError` は `400 Bad Request` として返却される。
...
> 補足（現行実装）: 指定した `student_attribute` のデータがDBに存在しない場合、空データ（0件・空配列）を返します。`all` を指定した場合は全属性の集計データを返します。
...
interface ResponseTrendItem {
  ...
  // 現行実装では属性別集計が存在しない場合、各属性は0として返却される
  breakdown?: {
...
interface CategorySummaryItem {
  category: CommentCategory;    // content / material / operation / instructor / other
  count: number;                // コメント数
}
```
...
```json
{
  ...
  "category_summary": [
    ...
    { "category": "material", "count": 67 },
    { "category": "instructor", "count": 30 },
    { "category": "operation", "count": 45 },
...
**エンドポイント**: `GET /lectures/{lecture_id}/analysis`
...
> 補足（現行実装）: 指定した `student_attribute` のデータがDBに存在しない場合、空データ（response_count=0、空配列）を返します。`all` を指定した場合は全属性の集計データを返します。
...
interface SessionAnalysisResponse {
  ...
  fix_difficulty: { easy: number; hard: number }; // 修正難易度別件数（常に両キーを含む。0件の場合は0）
...
interface SessionNPS {
  ...
  detractors_percentage: number;// 批判者割合
  total_responses: number | null; // 総回答数（null の場合あり）
}
...
interface CommentItem {
  ...
  importance: string | null;    // 重要度（通常は high / medium / low）
  fix_difficulty: string | null;  // 修正難易度（通常は easy / hard / none）
...
> 補足（現行実装）: 指定した `student_attribute` のデータがDBに存在しない場合、各メトリクスは0件・空データとなります。`all` を指定した場合は全属性の集計データで比較します。
...
interface YearMetrics {
  ...
  average_scores: Record<string, number>; // 例: overall_satisfaction など。データなし時は {}
}
...
CSV / Excel（`.xlsx`）ファイルをアップロードし、アンケートデータを登録する。
...
> 補足（現行実装）: Excel形式は `.xlsx` のみ対応（`.xls` は未対応）。
> 補足（現行実装）: CSV / Excel のヘッダー検証エラー（`（任意）` / `【必須】` 列なし等）は `400 Bad Request` を返す。`batch_type` 必須項目不足などの通常入力エラーも同様に `400` を返す。
...
**エンドポイント**: `DELETE /surveys/batches/{batch_id}`
...
> 補足（現行実装）: ALBヘッダーが無い直接アクセス等では `null` を返す場合がある。
...
ログインフローを開始する。ALBがCognitoのHosted UIへリダイレクトし、認証成功後にフロントエンドのトップページへリダイレクトする。

**エンドポイント**: `GET /login`

**処理フロー**

1. ブラウザが `/api/v1/login` へアクセス
2. ALBが未認証を検知し、CognitoのHosted UIへ302リダイレクト
3. ユーザーがCognitoでID/パスワードを入力
4. 認証成功後、ALBがセッションCookie（`AWSELBAuthSessionCookie`）を発行
5. FastAPIがフロントエンドのトップページ（`/`）へ307リダイレクト

**レスポンス**

- **307 Redirect**: 認証成功時、フロントエンドのトップページへリダイレクト
...
function redirectToLogin() {
  // fetchではリダイレクトを追跡できないため、ブラウザ全体を遷移させる
...
ログアウト処理を実行する。ブラウザのセッションCookieを削除し、Cognito設定がある場合はログアウトエンドポイントへリダイレクトしてAWS側セッションも終了させる。Cognito設定が未指定の場合は `/` へリダイレクトする。

**エンドポイント**: `GET /logout`

**処理フロー**

1. ブラウザが `/api/v1/logout` へアクセス
2. FastAPIがCookie削除のSet-Cookieヘッダーを付与
3. Cognito設定がある場合: CognitoのログアウトURL（`https://[domain]/logout?client_id=...&logout_uri=...`）へ302リダイレクト
4. Cognito設定がある場合: Cognito側のセッションが終了し、トップページ（`/`）へリダイレクト
5. Cognito設定がない場合: FastAPIが直接トップページ（`/`）へ302リダイレクト

**レスポンス**

- **302 Redirect**: Cognito設定ありの場合は Cognito ログアウトエンドポイントへ、未設定の場合は `/` へリダイレクト
...
- **4.5.1 ジョブ状態確認**
    ...
    **エンドポイント:** `GET /jobs/{job_id}`
    
    **パスパラメータ:** `job_id` (string) - `survey_batches.id` を文字列化した識別子
    ...
    **レスポンス**
    ...
    interface JobStatusResponse {
      ...
      created_at: string;
      // status='completed' の場合は結果オブジェクト、それ以外は null
      result: {
        lecture_id: number;
        batch_id: number;
        response_count: number;
      } | null;
      // 将来の failed 拡張用（現行実装では通常 null）
      error: {
        code: string;
        message: string;
      } | null;
...
> 注記: 認証列は本番運用（ALB + Cognito 経由）を前提にした記載。FastAPI単体では認証拒否を実装していない。
...
| GET | `/api/v1/jobs/{job_id}` | ジョブ状態確認（ポーリング用） | 必要 |
| GET | `/api/v1/lectures/{lecture_id}/analysis` | 講義回別分析データ取得 | 必要 |
| GET | `/api/v1/login` | ログイン（Cognitoへリダイレクト） | 不要 |
| GET | `/api/v1/logout` | ログアウト（セッション破棄） | 必要 |
| GET | `/api/v1/me` | ログインユーザー情報取得 | 必要 |
| POST | `/api/v1/surveys/upload` | アンケートデータアップロード | 必要 |
| GET | `/api/v1/surveys/batches/search` | 削除対象バッチ検索 | 必要 |
| DELETE | `/api/v1/surveys/batches/{batch_id}` | アンケートバッチ削除 | 必要 |
