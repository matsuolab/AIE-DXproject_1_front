...
export function getDummySessionAnalysis(lectureId: number): SessionAnalysisResponse {
  ...
  return {
    ...
    fix_difficulty: { easy: 10, hard: 4 },
    // priority_comments: meeting_priority=1 のコメント
    // 規則1: ネガティブ + 運営/資料 + high + easy
    priority_comments: [
      {
        id: 'demo-c2',
    ...
    comments: [
      ...
      // 優先度2: high + easy（優先度1の条件に該当しないもの）
      {
        id: 'demo-c10',
      ...
      // 優先度7: low + hard（Negativeのみ）
      {
        id: 'demo-c13',
        text: '他の受講者との交流機会が少なく、孤立感がありました。',
        sentiment: 'negative',
        category: 'other',
        importance: 'low',
        fix_difficulty: 'hard',
